...
class Query implements \IteratorAggregate, \Countable
{
     ...
     * @chainable
     */
    public function where($sql = null, $params = [])
    {
     ...
     * @chainable
     */
    public function andWhere($sql = null, $params = [])
    {
     ...
     * @chainable
     */
    public function orWhere($sql = null, $params = [])
    {
        if (!isset($this->_where)) {
            $this->_where = new Criteria();
        }
     ...
     * @chainable
     */
    public function having($sql = null, $params = [])
    {
     ...
     * @chainable
     */
    public function groupBy($sql, $params = [])
    {
        $binder = new Binder();
     ...
     * @chainable
     */
    public function andOrderBy($sql, $params = [])
    {
        $binder = new Binder();
        $this->_order[] = $binder->magicBind($sql, (array) $params);
    ...
    {
        if (!in_array($method, ['row', 'scalar', 'column'])) {
            throw new \BadMethodCallException("$method not implemented on Query or ResultSet");
        }

        return call_user_func_array([$this->execute(), $method], $params);
    }
