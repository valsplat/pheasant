...
class Binder
{
    ...
    {
        return $this->_bindInto('\?', $sql, $params, function ($binder, $param) {
     ...
     * @return string
     */
    public function magicBind($sql, array $params = [])
    {
        return $this->_bindInto('(?:`\w+`|\w+)\s*(?:!=|=|<>)\s*\?|\?', $sql, $params, function ($binder, $param, $token) {
            if ($token == '?') {
                return $binder->sqlValue($param);
            } else {
